...
import { getWritingArticlesEngagement } from "@/services/analytics/writingAnalyticsService";
import { parsePagination } from "@/lib/analytics/metrics-core";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const sort =
      sortParam === "lifetime" || sortParam === "publishedAt" || sortParam === "views"
        ? sortParam
        : "views";
    ...
    const data = await getWritingArticlesEngagement(db, {
    ...
    logger.error({ msg: "GET /api/analytics/writing/articles", error: message });
